...
include('includes/dbconnection.php');   //Includes a file that establishes a connection to a database.


if(isset($_POST['login']))
  {
    $emailcon=$_POST['emailcont'];
    $password=md5($_POST['password']);  // Hashes the password using the MD5 algorithm
    $query=mysqli_query($con,"select ID from tbluser where  (Email='$emailcon' || MobileNumber='$emailcon') && Password='$password' ");
    $ret=mysqli_fetch_array($query);//Fetches the result of the query as an array.
    if($ret>0){
      $_SESSION['fosuid']=$ret['ID'];   //If the query returns a result, it sets the $_SESSION['fosuid'] variable to the user's ID and redirects the user to index.php.
                                        //If the query doesn't return a result, it sets an error message $msg to "Invalid Details.".
