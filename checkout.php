...
$orderno= mt_rand(100000000, 999999999);
        ...
        // Prepare update and insert queries using prepared statements
        $query_update_orders = "UPDATE tblorders SET OrderNumber=?, IsOrderPlaced='1', CashonDelivery=? WHERE UserId=? AND IsOrderPlaced IS NULL";
        $query_insert_address = "INSERT INTO tblorderaddresses(UserId, Ordernumber, Flatnobuldngno, StreetName, Area, Landmark, City) VALUES (?, ?, ?, ?, ?, ?, ?)";
        $query_update_quantity = "UPDATE tblfood SET ItemQty = ItemQty - 1 WHERE CategoryName=? AND ItemName=?";

        // Prepare and bind parameters for update query
        if ($stmt = mysqli_prepare($con, $query_update_orders)) {
            mysqli_stmt_bind_param($stmt, "isi", $orderno, $cod, $userid);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_close($stmt);
        }

        // Prepare and bind parameters for insert query
        if ($stmt = mysqli_prepare($con, $query_insert_address)) {
            mysqli_stmt_bind_param($stmt, "issssss", $userid, $orderno, $fnaobno, $street, $area, $lndmark, $city);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_close($stmt);
        }

        // Prepare and bind parameters for quantity update query
        foreach ($_POST['itemname'] as $itemname => $quantity) {
            if ($stmt = mysqli_prepare($con, $query_update_quantity)) {
                mysqli_stmt_bind_param($stmt, "iss", $quantity, $_POST['fcat'][$itemname], $itemname);
                mysqli_stmt_execute($stmt);
                mysqli_stmt_close($stmt);
            }
        }
        ...
        if (mysqli_commit($con)) {
            ...
            echo '<script>alert("Error placing your order. Please try again later.")</script>';
